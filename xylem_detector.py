...
def extendContrast(img, saturation = 10):
...
def clickContour(event,x,y,flags,param): 
...
def findXylem(img): # The function to extract xylem areas
    ...
    climg = extendContrast(img) # Add contrast to the image
    ...
    while True:
        ...
        if set_threshold==True: # This has to come after the thresholding as that is needed every loop but only needs to be shown if wanted
            ...
            set_percentile = 99 # Use a default 99th percentile, allowing for error cells to be removed
        ...
        while satisfied_bool == False:
            ...
            cv2.namedWindow('Selected Contour Image', cv2.WINDOW_NORMAL)
            cv2.setMouseCallback('Selected Contour Image',clickContour)
