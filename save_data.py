...
def save_xylem(dataframe):
    ...
    try:
        if os.path.isfile(str(os.path.dirname(__file__))+ "/data_save_location.txt") == True:
            with open(str(os.path.dirname(__file__))+ "/data_save_location.txt", "r+") as file:
            ...
            with open(str(os.path.dirname(__file__))+"/data_save_location.txt","w+") as file:
                file.write(save_path)
                file.close()
...
def save_analysis(dataframe):
    ...
    # Use the try form because if the path is entered incorrectly it won't run
    try:
        if os.path.isfile(str(os.path.dirname(__file__))+ "/save_location_analysis.txt") == True:
            with open(str(os.path.dirname(__file__))+ "/save_location_analysis.txt", "r+") as file:
                old_path = file.readline()
                old_path = old_path.lstrip('\x00')
                if save_path.lower() ==  old_path:
                    print("dataframe saved to original folder")
                    dataframe.to_csv(old_path +"/"+ file_name + ".csv")
                    file.close()
                else:
                    print("data frame saved to new folder")
                    dataframe.to_csv(save_path +"/"+ file_name + ".csv")
                    file.truncate(0)
                    file.write(save_path)
                    file.close()
                    # Update folder for next time       
        else:
            with open(str(os.path.dirname(__file__))+"/save_location_analysis.txt","w+") as file:
                file.write(save_path)
                file.close()
                print("dataframe saved to new folder")
                dataframe.to_csv(save_path +"/"+file_name + ".csv")
        ...
        if not os.path.exists(os.path.dirname(__file__)+"/datadump"):
            os.makedirs(os.path.dirname(__file__)+"/datadump")
        dataframe.to_csv(os.path.dirname(__file__)+"/datadump/" + file_name + ".csv")
